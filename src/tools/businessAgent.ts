...
import {
  ...
  rankBuyersLogic,
} from "./buyerDiscovery.js";
import {
  findTransportLogic,
  findSharedShipmentLogic,
  calculateCostLogic,
  type ProductType,
} from "./logisticsOptimization.js";
import {
  generateNegotiationLogic,
  priceSuggestionLogic,
  shouldNegotiateLogic,
  draftContractLogic,
} from "./negotiationAssistant.js";
import {
  findSupplierLogic,
  findRareProductLogic,
  comparePricesLogic,
  estimateDeliveryLogic,
} from "./inventoryMatching.js";
...
type Intent = "sell_product" | "find_rare_item" | "reduce_transport_cost" | "general_query";
...
function classifyIntent(prompt: string): ParsedIntent {
...
async function runSellProductFlow(parsed: ParsedIntent): Promise<string> {
  ...
  if (transportOptions.length > 0) {
    ...
    if (negotiationAdvice.shouldNegotiate) {
      negotiationPlan = generateNegotiationLogic(
        topTransport.estimatedCost,
        avgRate,
        topTransport.vehicleType,
...
async function runFindRareItemFlow(parsed: ParsedIntent): Promise<string> {
  ...
  if (topSupplier) {
    deliveryEstimate = await estimateDeliveryLogic(
      topSupplier.location,
      location,
      quantity,
      productType,
      topSupplier.leadTimeDays
    );
  }
...
async function runReduceTransportCostFlow(parsed: ParsedIntent): Promise<string> {
...
export function registerBusinessAgentTools(server: McpServer): void {
  // compareAndRecommend as a standalone MCP tool
  server.tool(
    "compareAndRecommend",
    "Decision support: takes any list of options (buyers/suppliers/transport), computes weighted scores, detects near-ties within 10% on price, and returns plain-language trade-off explanations.",
    {
    ...
    async ({ options, criteria }) => {
      const result = compareAndRecommend(options as DecisionOption[], criteria ?? ["price", "distance"]);
  ...
  server.tool(
    ...
    `The main Anvaya Business Agent. Accepts a plain-language business intent and orchestrates ALL tool groups end-to-end.
      ...
      switch (parsed.intent) {
        ...
        default:
          result = JSON.stringify({
            ...
            message:
              "I couldn't determine a specific intent. Try phrasing like: 'I manufacture X and want to sell in Y', 'I need to source Z urgently', or 'Help me reduce transport costs from A to B'.",
            parsed,
          });
      }
