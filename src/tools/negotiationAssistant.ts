...
const ratesConfig = JSON.parse(
  ...
  negotiation: {
    openingOfferMultiplier: number;
    maxAcceptableMultiplier: number;
    floorMultiplier: number;
    incrementStep: number;
  };
};

const { openingOfferMultiplier, maxAcceptableMultiplier, floorMultiplier, incrementStep } =
  ratesConfig.negotiation;
...
export interface NegotiationPlan {
  ...
  incrementStep: number;
  messageDraft: string;
  expectedSavings: { amount: number; percentage: number };
}

export function generateNegotiationLogic(
  currentCost: number,
...
export function generateMessageLogic(
  openingOffer: number,
  context: {
    currentCost: number;
    averageMarketRate: number;
...
export function priceSuggestionLogic(
  currentCost: number,
  averageMarketRate: number
): { offer: number; maxAcceptable: number; expectedSavings: { amount: number; percentage: number } } {
...
export function shouldNegotiateLogic(
  currentCost: number,
  averageMarketRate: number,
  dealerRating: number
...
export function registerNegotiationTools(server: McpServer): void {
  server.tool(
    "generateNegotiation",
    "Generate a full negotiation strategy: opening offer, max acceptable price, floor, and expected savings. All logic is rule-based using constants from config/rates.json.",
    {
    ...
    async ({ currentCost, averageMarketRate, vehicleType, buyerDetails }) => {
      const plan = generateNegotiationLogic(
        currentCost,
        averageMarketRate,
  ...
  server.tool(
    "generateMessage",
    "Generate a ready-to-send negotiation message from a template using your opening offer and context.",
    {
  ...
  server.tool(
    "callSummary",
    "Generate a structured summary of a completed negotiation for logging and records.",
    {
  ...
  server.tool(
    "priceSuggestion",
    "Quick price suggestion: returns suggested opening offer, max acceptable price, and expected savings.",
    {
      currentCost: z.number().describe("Current or quoted cost in INR"),
  ...
  server.tool(
    "shouldNegotiate",
    "Analyzes whether you should negotiate a deal or accept it. Compares the quoted price against market rate and dealer rating to recommend action.",
    {
  ...
  server.tool(
    "draftContract",
    "Draft a long-term supply contract (6-month or 1-year) with lock-in pricing discounts. Generates a ready-to-sign contract document.",
    {
