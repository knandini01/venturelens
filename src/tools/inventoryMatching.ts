...
import { datasetService, type Supplier } from "../services/DatasetService.js";
import { compareAndRecommend, type DecisionOption } from "./decisionSupport.js";
import { calculateCostLogic, findTransportLogic, type ProductType } from "./logisticsOptimization.js";
...
export async function findSupplierLogic(
  productName: string,
  quantity: number,
  location: string,
  radiusKm = 500
...
export async function findRareProductLogic(productName: string): Promise<SupplierResult[]> {
  const all = datasetService.getSuppliers();
...
export function comparePricesLogic(suppliers: SupplierResult[]): {
  comparison: ReturnType<typeof compareAndRecommend>;
  sorted: SupplierResult[];
} {
  const options: DecisionOption[] = suppliers.map(s => ({
    id: s.id,
...
export async function estimateDeliveryLogic(
  supplierLocation: string,
  destination: string,
  cargoWeightKg: number,
  productType: ProductType = "standard",
  leadTimeDays = 2
): Promise<{ distanceKm: number; durationHrs: number; deliveryCost: number; totalDays: number; routeSource: string }> {
...
export function registerInventoryTools(server: McpServer): void {
  server.tool(
    "findSupplier",
    "Find suppliers for a product near a location, with distance, pricing, delivery estimate, and certifications.",
    {
  ...
  server.tool(
    "findRareProduct",
    "Fallback: search entire supplier dataset for a rare/specialty product regardless of location.",
    {
  ...
  server.tool(
    "comparePrices",
    "Compare supplier prices using the shared decision-support engine. Detects near-ties and provides trade-off explanation.",
    {
    ...
    async ({ suppliers }) => {
      const result = comparePricesLogic(suppliers as SupplierResult[]);
      return {
        content: [{
          type: "text",
          text: JSON.stringify({
  ...
  server.tool(
    "estimateDelivery",
    "Estimate delivery cost and time from a supplier to your location, using Logistics tools for route and cost.",
    {
    ...
    async ({ supplierLocation, destination, cargoWeightKg, productType, leadTimeDays }) => {
      const result = await estimateDeliveryLogic(
        supplierLocation,
        destination,
        cargoWeightKg,
