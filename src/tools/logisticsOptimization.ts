...
import { datasetService, type Transport } from "../services/DatasetService.js";
...
const ratesConfig = JSON.parse(
  ...
  vehicleRates: Record<string, { baseRate: number; perKmRate: number; perKgRate: number }>;
  productTypeMultipliers: Record<string, number>;
};
...
export function calculateCostLogic(
  distanceKm: number,
  cargoWeightKg: number,
  vehicleType: string,
  productType: ProductType = "standard"
): { baseCost: number; finalCost: number; multiplier: number; breakdown: Record<string, number> } {
...
function vehicleSupportsProduct(vehicle: Transport, productType: ProductType): boolean {
  if (productType === "perishable" && !vehicle.supportsRefrigeration) return false;
  if (productType === "fragile" && !vehicle.supportsFragile) return false;
  if (productType === "hazardous" && !vehicle.supportsHazardous) return false;
  return true;
}

// ── Core Logic (exported for orchestrator) ────────────────────────────────────

export async function findTransportLogic(
  pickupLocation: string,
  destination: string,
  cargoWeightKg: number,
  productType: ProductType,
  preferredDate?: string
): Promise<Array<Transport & { distanceKm: number; estimatedCost: number; costBreakdown: Record<string, number> }>> {
  ...
  return all
    .filter(t => {
      // Capacity check
      if (t.capacityKg < cargoWeightKg) return false;
      // Product type capability
      if (!vehicleSupportsProduct(t, productType)) return false;
      // Loose location match (fixture matching by city name)
      const srcMatch =
        t.source.toLowerCase().includes(pickupLocation.toLowerCase()) ||
...
export async function findSharedShipmentLogic(
  pickupLocation: string,
  destination: string,
  cargoWeightKg: number,
  productType: ProductType,
  preferredDate?: string
): Promise<Array<Transport & { distanceKm: number; sharedCost: number }>> {
  const [pickupGeo, destGeo] = await Promise.all([geocode(pickupLocation), geocode(destination)]);
  const all = datasetService.getTransport();

  return all
    .filter(t => {
      if (t.spareCapacityKg < cargoWeightKg) return false;
      if (!vehicleSupportsProduct(t, productType)) return false;
      const geoMatch = pickupGeo && destGeo
...
export function registerLogisticsTools(server: McpServer): void {
  server.tool(
    "findTransport",
    "Find available transport providers matching your route, cargo weight, and product type (standard/perishable/fragile/hazardous). Filters by capability flags and estimates cost.",
    {
    ...
    async ({ pickupLocation, destination, cargoWeightKg, productType, preferredDate }) => {
      const vehicles = await findTransportLogic(
        pickupLocation, destination, cargoWeightKg,
  ...
  server.tool(
    "calculateCost",
    "Calculate transport cost using the formula: (baseRate + distanceKm*perKmRate + cargoWeightKg*perKgRate) * productTypeMultiplier. Rates come from config/rates.json.",
    {
    ...
    async ({ distanceKm, cargoWeightKg, vehicleType, productType }) => {
      const result = calculateCostLogic(distanceKm, cargoWeightKg, vehicleType, (productType ?? "standard") as ProductType);
  ...
  server.tool(
    "routeOptimization",
    "Geocode pickup and destination via Nominatim, get driving distance/duration via OSRM (falls back to Haversine if unavailable).",
    {
  ...
  server.tool(
    "findSharedShipment",
    "Find vehicles already scheduled on an overlapping route with enough spare capacity for your cargo — enables cost sharing.",
    {
    ...
    async ({ pickupLocation, destination, cargoWeightKg, productType, preferredDate }) => {
      const options = await findSharedShipmentLogic(
        pickupLocation, destination, cargoWeightKg,
        (productType ?? "standard") as ProductType,
        preferredDate
      );
  ...
  server.tool(
    "estimateSavings",
    "Calculate savings between solo transport cost and shared shipment cost.",
    {
