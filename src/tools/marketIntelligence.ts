...
) as { categoryToShopTags: Record<string, string[]>; demandScoring: Record<string, number> };

// ── Helpers ──────────────────────────────────────────────────────────────────

function categoryToTags(productCategory: string): string[] {
...
function inferCategory(productName: string): string {
  if (isAgriculturalProduct(productName)) return "agri";
  if (/soap|detergent|shampoo|oil|biscuit|snack|food/i.test(productName)) return "fmcg";
  if (/cloth|fabric|silk|cotton|textile/i.test(productName)) return "textiles";
  if (/tablet|capsule|medicine|pharma/i.test(productName)) return "pharma";
  if (/phone|laptop|electronic/i.test(productName)) return "electronics";
  if (/craft|art|pottery|handmade|handicraft/i.test(productName)) return "handicrafts";
  return "general";
}
...
interface MarketResult {
  ...
  nearbyBusinesses: Array<{ name: string; distanceKm: number; type: string }>;
  estimatedSellingPrice: { min: number; max: number; currency: string; basis: "live" | "estimated" };
...
export async function searchMarketLogic(
  productName: string,
  businessType: string,
  ...
  // Demand score
  const { newsWeightAgri, newsMentionMax, competitorDensityMax, priceStabilityBonus } =
    ratesConfig.demandScoring;
  ...
  const competitionScore = Math.min(
    100,
    Math.round((osmShops.length / competitorDensityMax) * 100)
  );

  // Selling price
  let priceRange: { min: number; max: number; basis: "live" | "estimated" };
  if (commodityPrice) {
    priceRange = {
  ...
  const confidenceScore = Math.round(
    40 + (hasGeoData ? 25 : 0) + (hasLivePrice ? 25 : 10) + (hasNews ? 10 : 0)
  );
...
export function registerMarketIntelligenceTools(server: McpServer): void {
  // 1. searchMarket
  server.tool(
    "searchMarket",
    "Analyze whether a product can be sold profitably in a target market. Geocodes location, finds nearby businesses via OpenStreetMap, fetches commodity prices (Agmarknet for agri), and sentiment (NewsAPI).",
    {
  ...
  server.tool(
    "analyzeDemand",
    "Derive a 0-100 demand score for a product in a location using news mentions, commodity price trends, and business density.",
    {
  ...
  server.tool(
    "findCompetitors",
    "Find nearby competing businesses via OpenStreetMap Overpass API within a given radius.",
    {
  ...
  server.tool(
    ...
    "Estimate a selling price range for a product. Uses Agmarknet live price for agricultural products; rule-based estimate for others.",
    {
      productName: z.string().describe("Product name"),
  ...
  server.tool(
    "rankMarkets",
    "Rank multiple candidate locations for selling a product. Runs full market analysis per location and returns a ranked recommendation.",
    {
      productName: z.string().describe("Product name"),
    ...
    async ({ productName, businessType, candidateLocations, budget }) => {
      ...
      const options: DecisionOption[] = results.map(r => ({
        id: r.location,
        label: r.location,
