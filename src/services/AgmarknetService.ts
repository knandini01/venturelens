...
export async function getCommodityPrice(
  ...
  try {
    ...
    if (!res.ok) return getEstimatedPrice(commodity);
    ...
    if (!json.records?.length) return getEstimatedPrice(commodity);
...
function getEstimatedPrice(commodity: string): CommodityPrice | null {
  const key = Object.keys(ESTIMATED_PRICES).find(k =>
    commodity.toLowerCase().includes(k)
  );
  if (!key) return null;
...
export function isAgriculturalProduct(productOrCategory: string): boolean {
  const agriKeywords = [
