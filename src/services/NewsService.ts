...
export async function getNewsSentiment(
  ...
  try {
    ...
    if (!res.ok) return getMockSentiment(query, productName);
    ...
    POSITIVE_WORDS.forEach(w => { if (allText.includes(w)) posScore++; });
    NEGATIVE_WORDS.forEach(w => { if (allText.includes(w)) negScore++; });
...
function getMockSentiment(query: string, product: string): NewsSentiment {
  // Seed a deterministic pseudo-score based on product name hash
  let hash = 0;
  for (const c of product) hash = (hash * 31 + c.charCodeAt(0)) & 0xffff;
