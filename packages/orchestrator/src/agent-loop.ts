...
import { McpPool, DiscoveredTool } from './mcp-pool.js';

// ── Types ────────────────────────────────────────

export interface WorkflowStep {
  type: 'tool_call' | 'tool_result' | 'thinking' | 'response' | 'confirmation_gate';
  toolName?: string;
  toolArgs?: Record<string, unknown>;
  result?: unknown;
  text?: string;
  confirmationData?: ConfirmationGate;
  timestamp: string;
}
...
export interface AgentResponse {
  ...
  status: 'completed' | 'awaiting_confirmation' | 'error';
}
...
function convertSchemaToGemini(schema: any): Schema {
  ...
  else if (schema.type === 'number') mapped.type = SchemaType.NUMBER;
  else if (schema.type === 'integer') mapped.type = SchemaType.INTEGER;
  else if (schema.type === 'boolean') mapped.type = SchemaType.BOOLEAN;
  else if (schema.type === 'array') mapped.type = SchemaType.ARRAY;
...
export class AgentLoop {
  ...
  constructor(pool: McpPool, apiKey: string, modelName = 'gemini-2.0-flash') {
    this.pool = pool;
    this.genAI = new GoogleGenerativeAI(apiKey);
    this.modelName = modelName;
  }

  private buildToolDefinitions(): FunctionDeclaration[] {
  ...
  async execute(userMessage: string): Promise<AgentResponse> {
    ...
    const pendingConfirmations: ConfirmationGate[] = [];
    ...
    let currentMessage: Part[] = [{ text: userMessage }];
      ...
      for (const call of functionCalls) {
        ...
        try {
          ...
          const simplifyContent = Array.isArray(mcpResult?.content)
            ? { text: mcpResult.content.map((c: any) => c.text ?? JSON.stringify(c)).join('\n') }
            : mcpResult;
