...
import { findRetailersLogic, findWholesalersLogic, findDistributorsLogic, rankBuyersLogic, } from "./tools/buyerDiscovery.js";
import { findTransportLogic, findSharedShipmentLogic, } from "./tools/logisticsOptimization.js";
import { generateNegotiationLogic, shouldNegotiateLogic, draftContractLogic, } from "./tools/negotiationAssistant.js";
import { findSupplierLogic, findRareProductLogic, comparePricesLogic, estimateDeliveryLogic, } from "./tools/inventoryMatching.js";
import { checkStockLevelLogic, updateInventoryLogic } from "./tools/inventoryManagement.js";
import { generateInvoiceLogic, logTransactionLogic } from "./tools/businessDocuments.js";
...
const sellProductSteps = [
    {
        ...
        handler: async (ctx) => {
            const parsed = classifyIntent(ctx.prompt);
    ...
    },
    {
        ...
        handler: async (ctx) => {
            const transport = ctx.transportOptions;
            if (!transport.length)
                return { negotiationAdvice: null, negotiationPlan: null, stepResult: { skipped: true } };
    ...
    },
    {
        ...
        handler: async (ctx) => {
            const topBuyers = ctx.topBuyers;
            const transport = ctx.transportOptions;
            const shared = ctx.sharedOptions;
            const market = ctx.marketResult;
    ...
    },
    {
        ...
        handler: async (ctx) => {
            ...
            const topBuyers = ctx.topBuyers;
            const transport = ctx.transportOptions;
            const inventoryItem = ctx.inventoryItem;
            const market = ctx.marketResult;
            const hasStock = ctx.hasStock;
            if (!topBuyers[0])
                return { invoice: null, stepResult: { skipped: true, reason: "No buyer found" } };
            ...
            if (hasStock)
                updateInventoryLogic(parsed.productName, parsed.quantity, parsed.location);
            return {
    ...
    },
    {
        ...
        handler: async (ctx) => {
            ...
            const market = ctx.marketResult;
            if (!topBuyers[0])
                return { contactMessage: null, stepResult: { skipped: true } };
...
const findRareItemSteps = [
    {
        id: "understand_intent", label: "Understanding business intent", icon: "🧠",
        subSteps: ["Parsing natural language", "Classifying intent", "Extracting parameters"],
        handler: async (ctx) => { const parsed = classifyIntent(ctx.prompt); return { parsed, stepResult: parsed }; },
    },
    {
        ...
        handler: async (ctx) => {
            ...
            let fallback = false;
            if (!suppliers.length) {
                suppliers = await findRareProductLogic(parsed.productName);
                fallback = true;
            }
    ...
    },
    {
        ...
        handler: async (ctx) => {
            ...
            if (!suppliers.length)
                return { priceComparison: null, topSupplier: null, stepResult: { skipped: true } };
            const comparison = comparePricesLogic(suppliers);
            return { priceComparison: comparison, topSupplier: comparison.sorted[0], stepResult: { recommended: comparison.comparison.recommended, sorted: comparison.sorted.slice(0, 3).map((s) => ({ name: s.name, location: s.location, unitPrice: s.unitPrice, unit: s.unit })) } };
        },
    },
    {
        ...
        handler: async (ctx) => {
            const parsed = ctx.parsed;
            const top = ctx.topSupplier;
            if (!top)
                return { deliveryEstimate: null, stepResult: { skipped: true } };
    ...
    },
    {
        ...
        handler: async (ctx) => {
            const suppliers = ctx.suppliers;
            const top = ctx.topSupplier;
            if (!top || suppliers.length < 2)
                return { negotiationAdvice: null, stepResult: { skipped: true } };
            const avg = suppliers.reduce((s, sup) => s + sup.unitPrice, 0) / suppliers.length;
    ...
    },
    {
        ...
        handler: async (ctx) => {
            ...
            const top = ctx.topSupplier;
            if (!top)
                return { contract: null, stepResult: { skipped: true } };
...
const reduceTransportSteps = [
    ...
    },
    {
        ...
        handler: async (ctx) => {
            ...
            if (!best)
                return { contactMessage: null, stepResult: { skipped: true } };
            const msg = contactDealerLogic({ dealerName: best.provider, dealerPhone: best.contactPhone ?? "+91-XXXXXXXXXX", dealerType: "transporter", productName: "Shipment", quantity: parsed.quantity, unit: "kg", proposedPrice: shared[0]?.sharedCost ?? transport[0]?.estimatedCost ?? 0, yourBusinessName: parsed.sellerName, yourLocation: parsed.location, channel: "email" });
            return { contactMessage: msg, stepResult: { to: msg.to, subject: msg.subject, body: msg.body } };
        },
    },
];
// ── HTTP Server ───────────────────────────────────────────────────────────────
function getStepsForIntent(intent) {
    switch (intent) {
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/api/execute-workflow" && req.method === "POST") {
        ...
        if (parsed.intent === "sell_product") {
            const b = ctx.topBuyers?.[0];
            const t = ctx.transportOptions?.[0];
            const m = ctx.marketResult;
            if (b) {
                ...
                if (ctx.invoice) {
                    finalAnswer += `I've automatically generated GST invoice <strong>${ctx.invoice.invoice.invoiceNumber}</strong> for this transaction, logged it to your ledger, and drafted a proposal email to the buyer.`;
                }
            }
            else {
            ...
            if (s) {
                ...
                if (ctx.deliveryEstimate) {
                    finalAnswer += `Estimated delivery takes about ${ctx.deliveryEstimate.totalDays} days, costing ₹${ctx.deliveryEstimate.deliveryCost}.<br><br>`;
                }
                if (ctx.contract) {
                    finalAnswer += `Since this is a good deal, I've drafted a <strong>6-month supply contract</strong> locking in a ${ctx.contract.discountPercent}% discount, and prepared an email to send them.`;
                }
            }
            else {
