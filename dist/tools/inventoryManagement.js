...
export function checkStockLevelLogic(productName) {
    const all = loadInventory();
    const filtered = productName
        ? all.filter(i => i.productName.toLowerCase().includes(productName.toLowerCase()) ||
...
export function updateInventoryLogic(productName, quantitySold, sellerLocation) {
    const items = loadInventory();
    const idx = items.findIndex(i => i.productName.toLowerCase().includes(productName.toLowerCase()) ||
        productName.toLowerCase().includes(i.productName.toLowerCase()));
    if (idx === -1) {
        return {
            updated: false,
...
export function addInventoryItemLogic(productName, productType, category, sellerName, sellerLocation, quantity, unit, unitCost, reorderLevel) {
    const items = loadInventory();
...
export function registerInventoryManagementTools(server) {
    server.tool("checkStockLevel", "Check current inventory levels. Optionally filter by product name. Returns items and any low-stock alerts where quantity is at or below the reorder threshold.", {
    ...
    server.tool("updateInventoryTracker", "Deduct sold quantity from inventory after a transaction. Automatically triggers low-stock alerts if quantity drops below the reorder level. Also updates the seller location.", {
    ...
    server.tool("addInventoryItem", "Add a new product to the inventory tracker with initial stock, reorder level, and seller details.", {
