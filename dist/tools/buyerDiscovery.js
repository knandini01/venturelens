...
function inferCategory(product) {
    if (/soap|detergent|shampoo|oil|biscuit|snack|food|fmcg/i.test(product))
        return "fmcg";
    if (/grain|wheat|rice|chilli|turmeric|agri|spice|vegetable|fruit|sugar|honey/i.test(product))
        return "agri";
    if (/cloth|fabric|silk|cotton|textile/i.test(product))
        return "textiles";
    if (/tablet|medicine|pharma/i.test(product))
        return "pharma";
    if (/craft|art|handicraft|handmade/i.test(product))
        return "handicrafts";
    return "general";
}
...
async function datasetBuyersNear(type, product, locationGeo, radiusKm) {
    const category = inferCategory(product);
    const all = datasetService.getBuyers();
    return all
        .filter(b => {
        if (b.businessType !== type)
            return false;
...
async function osmBuyersNear(shopTags, locationGeo, radiusKm, type) {
...
function mergeBuyers(osm, dataset) {
...
export async function findRetailersLogic(product, location, radiusKm) {
...
export async function findWholesalersLogic(product, location, radiusKm) {
    const geo = await geocode(location);
    if (!geo)
        return [];
    const [osm, dataset] = await Promise.all([
...
export async function findDistributorsLogic(product, location, radiusKm) {
    const geo = await geocode(location);
    if (!geo)
        return [];
    return datasetBuyersNear("distributor", product, geo, radiusKm);
}
export function rankBuyersLogic(buyers, quantity, product) {
...
export function estimateSalesLogic(buyer, quantity) {
...
export function registerBuyerDiscoveryTools(server) {
    server.tool("findRetailers", "Find retail shops likely to buy your product — merges OpenStreetMap OSM shop data with the internal buyer dataset.", {
    ...
    server.tool("findWholesalers", "Find wholesale buyers for your product — merges OSM wholesale tags with internal dataset.", {
        product: z.string().describe("Product name"),
        location: z.string().describe("Location to search in"),
    ...
    server.tool("findDistributors", "Find distributors for your product from the internal dataset (OSM has no reliable distributor tagging).", {
        product: z.string().describe("Product name"),
        location: z.string().describe("Location to search in"),
    ...
    server.tool("rankBuyers", "Rank a list of buyers by price, distance, volume, and reliability using the decision support engine.", {
        ...
        const ranked = rankBuyersLogic(buyers, quantity, product);
        return { content: [{ type: "text", text: JSON.stringify({ rankedBuyers: ranked }, null, 2) }] };
    });
    server.tool("estimateSales", "Estimate how many units a buyer can absorb and the resulting revenue.", {
        buyer: z.object({
            id: z.string(),
            name: z.string(),
            businessType: z.enum(["retailer", "wholesaler", "distributor"]),
