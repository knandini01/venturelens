...
const rates = JSON.parse(readFileSync(join(__dirname, "../../config/rates.json"), "utf-8"));
/**
 ...
 */
export function compareAndRecommend(options, criteria = ["price", "distance"]) {
    if (!options.length) {
        return {
    ...
    const nearTiedOptions = scored.filter(o => o.id !== top.id && Math.abs(o.price - top.price) <= threshold);
    const nearTies = nearTiedOptions.length > 0
...
function buildTradeoffNote(top, other) {
    ...
    if (typeof otherRel === "number" && typeof topRel === "number") {
        if (otherRel > topRel)
            parts.push(`with higher reliability (${otherRel} vs ${topRel})`);
        else if (otherRel < topRel)
            parts.push(`with lower reliability (${otherRel} vs ${topRel})`);
    }
    return parts.join(", ") + ".";
}
function buildReasoning(top, all, criteria) {
...
//# sourceMappingURL=decisionSupport.js.map